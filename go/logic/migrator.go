...
func (this *Migrator) initiateHooksExecutor() (err error) {
	// 为当前的迁移器 Migrator初始化hooksExecutor
	// 创建 hooksExecutor，它维护了 migrartionContext
...
// retryOperation最多尝试运行给定函数的“count”次尝试，返回后立即退出。
...
func (this *Migrator) Migrate() (err error) {
	...
	//解析变更sql,获得到将原始表名命名为新的表名 若出错 直接返回错误
	...
	// todo 初始化一个 applier （对gho表执行alter，记录日志）
...
func (this *Migrator) cutOver() (err error) {
    ...
    //todo 这个参数的意思是--test-on-replica，告诉ghost这是在预检查？？？
	if this.migrationContext.TestOnReplica {
		// With `--test-on-replica` we stop replication thread, and then proceed to use
		// the same cut-over phase as the master would use. That means we take locks
		// and swap the tables.
...
// The "inspeinitiateInspectorctor" server is typically a replica; it is where we issue some
// queries such as:
// - table row count
// - schema validation
// - heartbeat
...
// initiateInspector函数用于连接、检验、检验我们的"inspector" server.
// 所谓的"inspeinitiateInspectorctor" server通常来讲是集群中的 replica（从库），我们会在它身上执行诸如 table row count、schema validation、hearbeat等校验。
// 当我们有提供--allow-on-master时，inspector指的时主库
func (this *Migrator) initiateInspector() (err error) {
	this.inspector = NewInspector(this.migrationContext) // todo创建一个Inspector
...
// 实际执行row copy的函数
// iterateChunks迭代现有表行，并生成将大块行复制任务到幻影表上。
...
func (this *Migrator) iterateChunks() error {
	...
	for {
		...
		copyRowsFunc := func() error {
			...
			// 当hasFurterRange为false时，原始表可能被写锁定，CalculateNextIterationRangeEndValues将永远挂起
			// When hasFurtherRange is false,
			// original table might be write locked and CalculateNextIterationRangeEndValues would hangs forever
...
// executeWriteFuncs通过applier写入数据：rowcopy和事件积压。
//这是鬼表获取数据的地方。 该函数填充单线程数据。
//轮询事件积压和行复制事件； 积压事件具有优先权。
