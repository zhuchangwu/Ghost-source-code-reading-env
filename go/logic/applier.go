...
//Applier 连接并对数据迁移所在的服务器进行写入
//一般是主库 当然也可以是从库
//Applier 才是真正把数据从原始表写入幽灵表以及将bin log中的原始表的变更同步到幽灵表
//幽灵表以及变更日志在Applier生成, 重命名表名也发生在Applier
type Applier struct {
...
// CalculateNextIterationRangeEndValues读取下一个迭代范围结束的唯一键值，该键值将用于复制下一个行块。
// 如果没有更多的块需要处理，Ir返回“false”，即我们已经超过了最后一个块并完成了对范围的迭代（这是通过复制行块完成的）
...
// ApplyIterationInsertQuery 要解决的问题就是在幽灵表上执行chunk-Insert query。也就是实际从原表复制数据的地方
...
func (this *Applier) DropAtomicCutOverSentryTableIfExists() error {
	...
	tableName := this.migrationContext.GetOldTableName()// 检查 _user_del 表是否存在
